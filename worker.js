...
function evaluateAST(node){ if(node.type==='num') return node.value; if(node.value!==undefined) return node.value; let leftVal=node.left?evaluateAST(node.left):null; let rightVal=evaluateAST(node.right); const ck=`${node.operator}|${leftVal}|${rightVal}`; if(calculationCache.has(ck)) return calculationCache.get(ck); let result; if(self.wasmMath && self.wasmMath.isReady){ switch(node.operator){ case '+': result=operatorFlags['+']? self.wasmMath.add(leftVal,rightVal):NaN; break; case '-': result=operatorFlags['-']? self.wasmMath.sub(leftVal,rightVal):NaN; break; case '*': result=operatorFlags['*']? self.wasmMath.mul(leftVal,rightVal):NaN; break; case '/': result=operatorFlags['/'] && rightVal!==0? self.wasmMath.div(leftVal,rightVal):NaN; break; case '%': if(!operatorFlags['%']|| rightVal===0) return NaN; result= leftVal - rightVal*Math.floor(leftVal/rightVal); break; case '^': if(!operatorFlags['^']) return NaN; result = (leftVal===0 && rightVal<=0)? (rightVal===0?1:NaN) : self.wasmMath.pow(leftVal,rightVal); break; case '√': result= operatorFlags['√'] && rightVal>=0 ? self.wasmMath.sqrt(rightVal):NaN; break; case '!': result= operatorFlags['!'] && rightVal<=MAX_FACTORIAL_INPUT && rightVal>=0 && Number.isInteger(rightVal)? self.wasmMath.factorial(rightVal): NaN; break; case 'log': result = operatorFlags['log'] && rightVal>0 ? Math.log10(rightVal) : NaN; break; case '||': { if(!operatorFlags['||']) return NaN; if(!Number.isInteger(leftVal)||!Number.isInteger(rightVal)|| leftVal<0 || rightVal<0) return NaN; result = parseFloat(String(Math.trunc(leftVal)) + String(Math.trunc(rightVal))); break; } case '∑': { if(!operatorFlags['∑']) return NaN; if(!Number.isInteger(leftVal)||!Number.isInteger(rightVal)|| leftVal>rightVal) return NaN; const n= rightVal-leftVal+1; result = (leftVal+rightVal)*n/2; break; } } } else { switch(node.operator){ case '+': result=operatorFlags['+']? leftVal+rightVal:NaN; break; case '-': result=operatorFlags['-']? leftVal-rightVal:NaN; break; case '*': result=operatorFlags['*']? leftVal*rightVal:NaN; break; case '/': result=operatorFlags['/'] && rightVal!==0? leftVal/rightVal:NaN; break; case '%': if(!operatorFlags['%']|| rightVal===0) return NaN; result= leftVal - rightVal*Math.floor(leftVal/rightVal); break; case '^': if(!operatorFlags['^']) return NaN; result = (leftVal===0 && rightVal<=0)? (rightVal===0?1:NaN) : Math.pow(leftVal,rightVal); break; case '√': result= operatorFlags['√'] && rightVal>=0 ? Math.sqrt(rightVal):NaN; break; case '!': result= operatorFlags['!'] && rightVal<=MAX_FACTORIAL_INPUT && rightVal>=0 && Number.isInteger(rightVal)? factorial(rightVal): NaN; break; case 'log': result = operatorFlags['log'] && rightVal>0 ? Math.log10(rightVal) : NaN; break; case '||': { if(!operatorFlags['||']) return NaN; if(!Number.isInteger(leftVal)||!Number.isInteger(rightVal)|| leftVal<0 || rightVal<0) return NaN; result = parseFloat(String(Math.trunc(leftVal)) + String(Math.trunc(rightVal))); break; } case '∑': { if(!operatorFlags['∑']) return NaN; if(!Number.isInteger(leftVal)||!Number.isInteger(rightVal)|| leftVal>rightVal) return NaN; const n= rightVal-leftVal+1; result = (leftVal+rightVal)*n/2; break; } } } if(!isNaN(result)){ if(calculationCache.size>1000000) calculationCache.clear(); calculationCache.set(ck,result); node.value=result; } return result; }

function isIntegerResult(r){ return Number.isInteger(r) || Math.abs(r - Math.round(r)) < 0.0001; }
function serializeAST(node){ if(node.type==='num') return node.value.toString(); if(node.operator==='√') return `√(${serializeAST(node.right)})`; if(node.operator==='!') return `(${serializeAST(node.right)})!`; if(node.operator==='log') return `log(${serializeAST(node.right)})`; return `(${node.left?serializeAST(node.left):''} ${node.operator} ${serializeAST(node.right)})`; }
...
function generateAllGroupings(nums,target){
  ...
  function helper(start,end){
    ...
    if(start===end){
      ...
      if(operatorFlags['log'] && nums[start]>0){ let cur=numNode; for(let i=1;i<=MAX_LOG_DEPTH && operatorFlags['log'];i++){ cur={type:'op',operator:'log',left:null,right:cur}; const v=evaluateAST(cur); if(!isNaN(v) && (!useIntegerMode||isIntegerResult(v))) res.push(cur);} }
    } else {
